theory wp1 imports pt_general pexp
begin

section \<open>completing the language\<close>
...
primrec wptcc where 
  \<open>wptcc q (Assert r) i = (q i \<and> i \<^bold>\<Turnstile> r)\<close> |
  \<open>wptcc q (Var x) i = (\<forall> v. q (i(x := v)))\<close> |
  \<open>wptcc q (Assign x a) i = q (i(x := i\<lparr>a\<rparr>))\<close> 

(*lemma \<open>(\<^bold>[f :: \<^bold>('s \<Rightarrow> bool)\<^bold>] :: 's \<Rightarrow> bool) = f\<close>

outcome logic*)

fun wptc where (*weakest precondition for total correctness*)
  \<open>wptc q (\<bullet> c) = wptcc q c\<close> |
  \<open>wptc q (s1\<^bold>; s2) = wptc (wptc q s2) s1\<close> |
  \<open>wptc q (s1 \<^bold>[\<^bold>] s2) = wptc q s1 \<sqinter> wptc q s2\<close> |
  \<open>wptc q (If b s1 s2) = (\<lambda> i. if i \<^bold>\<Turnstile> b then i \<in> wptc q s1 else i \<in> wptc q s2)\<close> |
  \<open>wptc q (While b s) = lfp (\<lambda> x. {i. (i \<^bold>\<Turnstile> b \<longrightarrow> i \<in> wptc x s) \<and> (\<not>i \<^bold>\<Turnstile> b \<longrightarrow> i \<in> q)})\<close> | 
  \<open>wptc q (Ghost s) = wptc q s\<close> | 
  \<open>wptc q Skip = q\<close>
...
lemma wp_\<Phi>tc: \<open>wptc q (While b s) = lfp (\<Phi>tc b s q)\<close> 
  unfolding \<Phi>tc_def by simp

lemma wptcc_mono: \<open>mono (\<lambda> q. wptcc q c)\<close>
  by (rule monoI, induct c) auto
...
proof (rule monoI, induct rule: wptc.induct)
  case (1 q c)
  then show ?case 
    using wptcc_mono monoE by (metis wptc.simps(1))
next
  case (2 q s1 s2)
  then show ?case 
    using seq_mono
...
interpretation power_set_Fix_Lim_Ord \<open>{} :: (string \<Rightarrow> string \<Rightarrow> int) set\<close>
proof
  have \<open>infinite (UNIV :: (string \<Rightarrow> string \<Rightarrow> int) set)\<close> 
    by (meson finite_fun_UNIVD2 infinite_UNIV_char_0)
  then show \<open>infinite (UNIV :: (string \<Rightarrow> string \<Rightarrow> int) set set)\<close>
    by (simp add: Finite_Set.finite_set)
qed
